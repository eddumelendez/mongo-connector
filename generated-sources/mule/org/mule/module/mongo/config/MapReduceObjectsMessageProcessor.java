...
/**
 * MapReduceObjectsMessageProcessor invokes the {@link org.mule.module.mongo.MongoCloudConnector#mapReduceObjects(java.lang.String, java.lang.String, java.lang.String, java.lang.String)} method in {@link MongoCloudConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private String _mapFunctionType;
    private Object reduceFunction;
    private String _reduceFunctionType;
    private Object outputCollection;
    private String _outputCollectionType;
    private Object username;
    private String _usernameType;
    private Object password;
    private String _passwordType;
    private static Logger logger = LoggerFactory.getLogger(MapReduceObjectsMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("mapReduceObjects"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            if (username!= null) {
                transformedUsername = ((String) evaluateAndTransform(_muleMessage, MapReduceObjectsMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, username));
            } else {
                if (castedModuleObject.getUsername() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("mapReduceObjects"), event, new RuntimeException("You must provide a username at the config or the message processor level."));
                }
                transformedUsername = ((String) evaluateAndTransform(_muleMessage, MapReduceObjectsMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, castedModuleObject.getUsername()));
            }
            if (password!= null) {
                transformedPassword = ((String) evaluateAndTransform(_muleMessage, MapReduceObjectsMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, password));
            } else {
                if (castedModuleObject.getPassword() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("mapReduceObjects"), event, new RuntimeException("You must provide a password at the config or the message processor level."));
                }
                transformedPassword = ((String) evaluateAndTransform(_muleMessage, MapReduceObjectsMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, castedModuleObject.getPassword()));
            }
            String transformedCollection = ((String) evaluateAndTransform(_muleMessage, MapReduceObjectsMessageProcessor.class.getDeclaredField("_collectionType").getGenericType(), null, collection));
            String transformedMapFunction = ((String) evaluateAndTransform(_muleMessage, MapReduceObjectsMessageProcessor.class.getDeclaredField("_mapFunctionType").getGenericType(), null, mapFunction));
            String transformedReduceFunction = ((String) evaluateAndTransform(_muleMessage, MapReduceObjectsMessageProcessor.class.getDeclaredField("_reduceFunctionType").getGenericType(), null, reduceFunction));
            String transformedOutputCollection = ((String) evaluateAndTransform(_muleMessage, MapReduceObjectsMessageProcessor.class.getDeclaredField("_outputCollectionType").getGenericType(), null, outputCollection));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("mapReduceObjects"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.mapReduceObjects(transformedCollection, transformedMapFunction, transformedReduceFunction, transformedOutputCollection);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("mapReduceObjects"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
