    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private String _commandNameType;
    private Object commandValue;
    private String _commandValueType;
    private Object username;
    private String _usernameType;
    private Object password;
    private String _passwordType;
    private static Logger logger = LoggerFactory.getLogger(ExecuteCommandMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("executeCommand"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            if (username!= null) {
                transformedUsername = ((String) evaluateAndTransform(_muleMessage, ExecuteCommandMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, username));
            } else {
                if (castedModuleObject.getUsername() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("executeCommand"), event, new RuntimeException("You must provide a username at the config or the message processor level."));
                }
                transformedUsername = ((String) evaluateAndTransform(_muleMessage, ExecuteCommandMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, castedModuleObject.getUsername()));
            }
            if (password!= null) {
                transformedPassword = ((String) evaluateAndTransform(_muleMessage, ExecuteCommandMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, password));
            } else {
                if (castedModuleObject.getPassword() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("executeCommand"), event, new RuntimeException("You must provide a password at the config or the message processor level."));
                }
                transformedPassword = ((String) evaluateAndTransform(_muleMessage, ExecuteCommandMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, castedModuleObject.getPassword()));
            }
            String transformedCommandName = ((String) evaluateAndTransform(_muleMessage, ExecuteCommandMessageProcessor.class.getDeclaredField("_commandNameType").getGenericType(), null, commandName));
            String transformedCommandValue = ((String) evaluateAndTransform(_muleMessage, ExecuteCommandMessageProcessor.class.getDeclaredField("_commandValueType").getGenericType(), null, commandValue));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("executeCommand"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.executeCommand(transformedCommandName, transformedCommandValue);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("executeCommand"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
