...
import org.mule.module.mongo.processors.FindObjectsMessageProcessor;
    ...
    extends AbstractDevkitBasedDefinitionParser
{
    ...
    private static Logger logger = LoggerFactory.getLogger(FindObjectsDefinitionParser.class);

    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            return BeanDefinitionBuilder.rootBeanDefinition(FindObjectsMessageProcessor.class.getName());
            ...
            logger.error(("Cannot launch the mule app, the @Processor [find-objects] within the connector [mongo] is not supported in mule "+ muleVersion));
            throw new BeanDefinitionParsingException(new Problem(("Cannot launch the mule app, the @Processor [find-objects] within the connector [mongo] is not supported in mule "+ muleVersion), new Location(parserContext.getReaderContext().getResource()), null, noClassDefFoundError));
        }
    }

    public BeanDefinition parse(Element element, ParserContext parserContext) {
        BeanDefinitionBuilder builder = getBeanDefinitionBuilder(parserContext);
        builder.addConstructorArgValue("findObjects");
