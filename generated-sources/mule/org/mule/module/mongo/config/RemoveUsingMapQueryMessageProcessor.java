...
/**
 * RemoveUsingMapQueryMessageProcessor invokes the {@link org.mule.module.mongo.MongoCloudConnector#removeUsingMapQuery(org.mule.module.mongo.MongoSession, java.lang.String, java.util.Map, org.mule.module.mongo.api.WriteConcern)} method in {@link MongoCloudConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class RemoveUsingMapQueryMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
        ...
        throws MuleException
    {
        ...
        try {
            if (username!= null) {
                transformedUsername = ((String) evaluateAndTransform(muleMessage, RemoveUsingMapQueryMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), username));
            } else {
                transformedUsername = ((String) evaluateAndTransform(muleMessage, RemoveUsingMapQueryMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), moduleObject.getUsername()));
            }
            if (password!= null) {
                transformedPassword = ((String) evaluateAndTransform(muleMessage, RemoveUsingMapQueryMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), password));
            } else {
                transformedPassword = ((String) evaluateAndTransform(muleMessage, RemoveUsingMapQueryMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), moduleObject.getPassword()));
            }
            ...
            String transformedCollection = ((String) evaluateAndTransform(muleMessage, RemoveUsingMapQueryMessageProcessor.class.getDeclaredField("_collectionType").getGenericType(), collection));
            Map<String, String> transformedQueryAttributes = ((Map<String, String> ) evaluateAndTransform(muleMessage, RemoveUsingMapQueryMessageProcessor.class.getDeclaredField("_queryAttributesType").getGenericType(), queryAttributes));
            org.mule.module.mongo.api.WriteConcern transformedWriteConcern = ((org.mule.module.mongo.api.WriteConcern) evaluateAndTransform(muleMessage, RemoveUsingMapQueryMessageProcessor.class.getDeclaredField("_writeConcernType").getGenericType(), writeConcern));
            moduleObject.removeUsingMapQuery(session, transformedCollection, transformedQueryAttributes, transformedWriteConcern);
            return event;
        } catch (Exception e) {
            throw new MessagingException(CoreMessages.failedToInvoke("removeUsingMapQuery"), event, e);
        } finally {
            if (session!= null) {
                try {
