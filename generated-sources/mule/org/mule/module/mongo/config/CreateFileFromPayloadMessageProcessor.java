...
/**
 * CreateFileFromPayloadMessageProcessor invokes the {@link org.mule.module.mongo.MongoCloudConnector#createFileFromPayload(java.lang.Object, java.lang.String, java.lang.String, com.mongodb.DBObject)} method in {@link MongoCloudConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private String _filenameType;
    private Object contentType;
    private String _contentTypeType;
    private Object metadata;
    private DBObject _metadataType;
    private Object username;
    private String _usernameType;
    private Object password;
    private String _passwordType;
    private static Logger logger = LoggerFactory.getLogger(CreateFileFromPayloadMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("createFileFromPayload"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            if (username!= null) {
                transformedUsername = ((String) evaluateAndTransform(_muleMessage, CreateFileFromPayloadMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, username));
            } else {
                if (castedModuleObject.getUsername() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("createFileFromPayload"), event, new RuntimeException("You must provide a username at the config or the message processor level."));
                }
                transformedUsername = ((String) evaluateAndTransform(_muleMessage, CreateFileFromPayloadMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, castedModuleObject.getUsername()));
            }
            if (password!= null) {
                transformedPassword = ((String) evaluateAndTransform(_muleMessage, CreateFileFromPayloadMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, password));
            } else {
                if (castedModuleObject.getPassword() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("createFileFromPayload"), event, new RuntimeException("You must provide a password at the config or the message processor level."));
                }
                transformedPassword = ((String) evaluateAndTransform(_muleMessage, CreateFileFromPayloadMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, castedModuleObject.getPassword()));
            }
            Object transformedPayload = ((Object) evaluateAndTransform(_muleMessage, CreateFileFromPayloadMessageProcessor.class.getDeclaredField("_payloadType").getGenericType(), null, "#[payload]"));
            String transformedFilename = ((String) evaluateAndTransform(_muleMessage, CreateFileFromPayloadMessageProcessor.class.getDeclaredField("_filenameType").getGenericType(), null, filename));
            String transformedContentType = ((String) evaluateAndTransform(_muleMessage, CreateFileFromPayloadMessageProcessor.class.getDeclaredField("_contentTypeType").getGenericType(), null, contentType));
            DBObject transformedMetadata = ((DBObject) evaluateAndTransform(_muleMessage, CreateFileFromPayloadMessageProcessor.class.getDeclaredField("_metadataType").getGenericType(), null, metadata));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("createFileFromPayload"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.createFileFromPayload(transformedPayload, transformedFilename, transformedContentType, transformedMetadata);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("createFileFromPayload"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
