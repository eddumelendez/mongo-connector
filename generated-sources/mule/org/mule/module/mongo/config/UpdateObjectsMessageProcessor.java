    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private static Logger logger = LoggerFactory.getLogger(UpdateObjectsMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("updateObjects"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            if (username!= null) {
                transformedUsername = ((String) evaluateAndTransform(_muleMessage, UpdateObjectsMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, username));
            } else {
                if (castedModuleObject.getUsername() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("updateObjects"), event, new RuntimeException("You must provide a username at the config or the message processor level."));
                }
                transformedUsername = ((String) evaluateAndTransform(_muleMessage, UpdateObjectsMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, castedModuleObject.getUsername()));
            }
            if (password!= null) {
                transformedPassword = ((String) evaluateAndTransform(_muleMessage, UpdateObjectsMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, password));
            } else {
                if (castedModuleObject.getPassword() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("updateObjects"), event, new RuntimeException("You must provide a password at the config or the message processor level."));
                }
                transformedPassword = ((String) evaluateAndTransform(_muleMessage, UpdateObjectsMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, castedModuleObject.getPassword()));
            }
            String transformedCollection = ((String) evaluateAndTransform(_muleMessage, UpdateObjectsMessageProcessor.class.getDeclaredField("_collectionType").getGenericType(), null, collection));
            DBObject transformedQuery = ((DBObject) evaluateAndTransform(_muleMessage, UpdateObjectsMessageProcessor.class.getDeclaredField("_queryType").getGenericType(), null, query));
            DBObject transformedElement = ((DBObject) evaluateAndTransform(_muleMessage, UpdateObjectsMessageProcessor.class.getDeclaredField("_elementType").getGenericType(), null, element));
            Boolean transformedUpsert = ((Boolean) evaluateAndTransform(_muleMessage, UpdateObjectsMessageProcessor.class.getDeclaredField("_upsertType").getGenericType(), null, upsert));
            Boolean transformedMulti = ((Boolean) evaluateAndTransform(_muleMessage, UpdateObjectsMessageProcessor.class.getDeclaredField("_multiType").getGenericType(), null, multi));
            org.mule.module.mongo.api.WriteConcern transformedWriteConcern = ((org.mule.module.mongo.api.WriteConcern) evaluateAndTransform(_muleMessage, UpdateObjectsMessageProcessor.class.getDeclaredField("_writeConcernType").getGenericType(), null, writeConcern));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("updateObjects"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder messageStringBuilder = new StringBuilder();
            ...
            connection.updateObjects(transformedCollection, transformedQuery, transformedElement, transformedUpsert, transformedMulti, transformedWriteConcern);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
            throw new MessagingException(CoreMessages.failedToInvoke("updateObjects"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
