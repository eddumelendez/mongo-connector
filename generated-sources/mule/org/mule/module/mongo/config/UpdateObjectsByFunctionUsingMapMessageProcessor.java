...
/**
 * UpdateObjectsByFunctionUsingMapMessageProcessor invokes the {@link org.mule.module.mongo.MongoCloudConnector#updateObjectsByFunctionUsingMap(java.lang.String, java.lang.String, java.util.Map, java.util.Map, boolean, boolean, org.mule.module.mongo.api.WriteConcern)} method in {@link MongoCloudConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private Map<String, Object> _queryAttributesType;
    private Object elementAttributes;
    private Map<String, Object> _elementAttributesType;
    private Object upsert;
    private boolean _upsertType;
    private Object multi;
    private boolean _multiType;
    private Object writeConcern;
    private org.mule.module.mongo.api.WriteConcern _writeConcernType;
    private Object username;
    private String _usernameType;
    private Object password;
    private String _passwordType;
    private static Logger logger = LoggerFactory.getLogger(UpdateObjectsByFunctionUsingMapMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("updateObjectsByFunctionUsingMap"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            if (username!= null) {
                transformedUsername = ((String) evaluateAndTransform(_muleMessage, UpdateObjectsByFunctionUsingMapMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, username));
            } else {
                if (castedModuleObject.getUsername() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("updateObjectsByFunctionUsingMap"), event, new RuntimeException("You must provide a username at the config or the message processor level."));
                }
                transformedUsername = ((String) evaluateAndTransform(_muleMessage, UpdateObjectsByFunctionUsingMapMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, castedModuleObject.getUsername()));
            }
            if (password!= null) {
                transformedPassword = ((String) evaluateAndTransform(_muleMessage, UpdateObjectsByFunctionUsingMapMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, password));
            } else {
                if (castedModuleObject.getPassword() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("updateObjectsByFunctionUsingMap"), event, new RuntimeException("You must provide a password at the config or the message processor level."));
                }
                transformedPassword = ((String) evaluateAndTransform(_muleMessage, UpdateObjectsByFunctionUsingMapMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, castedModuleObject.getPassword()));
            }
            String transformedCollection = ((String) evaluateAndTransform(_muleMessage, UpdateObjectsByFunctionUsingMapMessageProcessor.class.getDeclaredField("_collectionType").getGenericType(), null, collection));
            String transformedFunction = ((String) evaluateAndTransform(_muleMessage, UpdateObjectsByFunctionUsingMapMessageProcessor.class.getDeclaredField("_functionType").getGenericType(), null, function));
            Map<String, Object> transformedQueryAttributes = ((Map<String, Object> ) evaluateAndTransform(_muleMessage, UpdateObjectsByFunctionUsingMapMessageProcessor.class.getDeclaredField("_queryAttributesType").getGenericType(), null, queryAttributes));
            Map<String, Object> transformedElementAttributes = ((Map<String, Object> ) evaluateAndTransform(_muleMessage, UpdateObjectsByFunctionUsingMapMessageProcessor.class.getDeclaredField("_elementAttributesType").getGenericType(), null, elementAttributes));
            Boolean transformedUpsert = ((Boolean) evaluateAndTransform(_muleMessage, UpdateObjectsByFunctionUsingMapMessageProcessor.class.getDeclaredField("_upsertType").getGenericType(), null, upsert));
            Boolean transformedMulti = ((Boolean) evaluateAndTransform(_muleMessage, UpdateObjectsByFunctionUsingMapMessageProcessor.class.getDeclaredField("_multiType").getGenericType(), null, multi));
            org.mule.module.mongo.api.WriteConcern transformedWriteConcern = ((org.mule.module.mongo.api.WriteConcern) evaluateAndTransform(_muleMessage, UpdateObjectsByFunctionUsingMapMessageProcessor.class.getDeclaredField("_writeConcernType").getGenericType(), null, writeConcern));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("updateObjectsByFunctionUsingMap"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder messageStringBuilder = new StringBuilder();
            ...
            connection.updateObjectsByFunctionUsingMap(transformedCollection, transformedFunction, transformedQueryAttributes, transformedElementAttributes, transformedUpsert, transformedMulti, transformedWriteConcern);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
            throw new MessagingException(CoreMessages.failedToInvoke("updateObjectsByFunctionUsingMap"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
