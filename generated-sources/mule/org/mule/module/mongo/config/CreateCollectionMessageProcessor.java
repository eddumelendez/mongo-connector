    ...
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{
    ...
    private Object capped;
    private boolean _cappedType;
    private Object maxObjects;
    private Integer _maxObjectsType;
    private Object size;
    private Integer _sizeType;
    private Object username;
    private String _usernameType;
    private Object password;
    private String _passwordType;
    private static Logger logger = LoggerFactory.getLogger(CreateCollectionMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws MuleException
    {
        ...
        if (moduleObject instanceof String) {
            ...
            if (castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("createCollection"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
        ...
        try {
            if (username!= null) {
                transformedUsername = ((String) evaluateAndTransform(_muleMessage, CreateCollectionMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, username));
            } else {
                if (castedModuleObject.getUsername() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("createCollection"), event, new RuntimeException("You must provide a username at the config or the message processor level."));
                }
                transformedUsername = ((String) evaluateAndTransform(_muleMessage, CreateCollectionMessageProcessor.class.getDeclaredField("_usernameType").getGenericType(), null, castedModuleObject.getUsername()));
            }
            if (password!= null) {
                transformedPassword = ((String) evaluateAndTransform(_muleMessage, CreateCollectionMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, password));
            } else {
                if (castedModuleObject.getPassword() == null) {
                    throw new MessagingException(CoreMessages.failedToCreate("createCollection"), event, new RuntimeException("You must provide a password at the config or the message processor level."));
                }
                transformedPassword = ((String) evaluateAndTransform(_muleMessage, CreateCollectionMessageProcessor.class.getDeclaredField("_passwordType").getGenericType(), null, castedModuleObject.getPassword()));
            }
            String transformedCollection = ((String) evaluateAndTransform(_muleMessage, CreateCollectionMessageProcessor.class.getDeclaredField("_collectionType").getGenericType(), null, collection));
            Boolean transformedCapped = ((Boolean) evaluateAndTransform(_muleMessage, CreateCollectionMessageProcessor.class.getDeclaredField("_cappedType").getGenericType(), null, capped));
            Integer transformedMaxObjects = ((Integer) evaluateAndTransform(_muleMessage, CreateCollectionMessageProcessor.class.getDeclaredField("_maxObjectsType").getGenericType(), null, maxObjects));
            Integer transformedSize = ((Integer) evaluateAndTransform(_muleMessage, CreateCollectionMessageProcessor.class.getDeclaredField("_sizeType").getGenericType(), null, size));
            ...
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("createCollection"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder messageStringBuilder = new StringBuilder();
            ...
            connection.createCollection(transformedCollection, transformedCapped, transformedMaxObjects, transformedSize);
            retryCount.set(0);
            return event;
        } catch (Exception e) {
            throw new MessagingException(CoreMessages.failedToInvoke("createCollection"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
